...
- IMPORTANT: After selecting the specific items you are recommending, you MUST call display_media with their TMDB IDs, media types, exact titles, and years copied from the prior tool results, ordered by relevance — that tool controls the visual carousel the user sees. Never invent or guess TMDB IDs. If display_media rejects an item as a mismatch, correct the ID from the prior search results before answering. Search results alone do NOT populate the carousel. Skip display_media for purely informational answers with no items to showcase.`
)
