...
func (f *fakeExecutor) Execute(ctx context.Context, issueID int64, kind ActionKind, params json.RawMessage, proposedAt time.Time, beforeQueueMutation ...func(string) error) (string, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
