...
func TestAllReleaseRenderersHideOpaqueAndSignedCapabilities(t *testing.T) {
	...
	lidarrRelease := lidarr.Release{GUID: "https://indexer.invalid/album?signature=lidarr-query-sentinel", IndexerID: 4, Title: "Lidarr Candidate"}
	...
	tests := []struct {
		...
		{name: "lidarr signed query", rawGUID: lidarrRelease.GUID, text: formatLidarrReleases([]lidarr.Release{lidarrRelease}), candidates: lidarrReleaseCandidates([]lidarr.Release{lidarrRelease})},
	}
...
// grab_release's music arm can only accept a reference that search_releases
// minted, so a search_releases that refused music left every album grab dead
// (via MCP and via the remediation agent, whose candidates bind to the search
// result). This drives the whole chain on a fake Lidarr.
func TestMusicReleaseSearchMintsTheReferenceGrabReleaseNeeds(t *testing.T) {
	const rawGUID = "https://indexer.invalid/album/opaque-signed-path-sentinel?token=capability-sentinel"
	const lidarrInstanceID = "lidarr-target"

	var searches atomic.Int32
	grabbedGUID := make(chan string, 1)
	target := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case r.Method == http.MethodGet && r.URL.Path == "/api/v1/release":
			searches.Add(1)
			if r.URL.Query().Get("albumId") != "77" {
				t.Errorf("release search query = %q", r.URL.RawQuery)
			}
			_ = json.NewEncoder(w).Encode([]map[string]any{{
				"guid": rawGUID, "indexerId": 9, "indexer": "Scoped Indexer", "title": "Scoped.Album.2026 " + rawGUID,
				"size": 1234, "protocol": "torrent", "seeders": 12, "leechers": 1, "ageHours": 48.0,
				"rejected": true, "rejections": []string{"untrusted metadata repeated " + rawGUID},
			}})
		case r.Method == http.MethodPost && r.URL.Path == "/api/v1/release":
			var body struct {
				GUID      string `json:"guid"`
				IndexerID int    `json:"indexerId"`
			}
	...
	defer database.Close()
	cipher, err := secrets.NewCipher(bytes.Repeat([]byte{0x53}, 32))
	if err != nil {
		t.Fatalf("NewCipher: %v", err)
	}
	store := instance.NewStore(database, cipher)
	if err := store.Create(&instance.Instance{ID: lidarrInstanceID, ServiceType: "lidarr", Name: "Music", URL: target.URL, APIKey: "target"}); err != nil {
		t.Fatalf("create instance: %v", err)
	}
	toolServer := NewToolServer(nil, nil, instance.NewRegistry(store), nil)
	callCtx := CallContext{Role: auth.RoleAdmin, TrustedInternal: true, InstanceID: lidarrInstanceID}

	// A music search is scoped by album_id alone; a stray title id is refused
	// before any indexer is asked.
	crossScoped, err := toolServer.ExecuteTool(context.Background(), "search_releases", json.RawMessage(`{"media_type":"music","album_id":77,"tmdb_id":42}`), callCtx)
	if err != nil {
		t.Fatalf("cross-scoped search_releases: %v", err)
	}
	if !strings.Contains(crossScoped.Text, "requires only a positive album_id") || searches.Load() != 0 {
		t.Fatalf("cross-scoped search = %q (searches %d), want a refusal before any search", crossScoped.Text, searches.Load())
	}

	searchResult, err := toolServer.ExecuteTool(context.Background(), "search_releases", json.RawMessage(`{"media_type":"music","album_id":77}`), callCtx)
	if err != nil {
		t.Fatalf("search_releases: %v", err)
	}
	reference := releaseGUIDReference(rawGUID)
	if len(searchResult.ReleaseCandidates) != 1 || searchResult.ReleaseCandidates[0].Reference != reference {
		t.Fatalf("release candidates = %+v, want the one-way reference %q", searchResult.ReleaseCandidates, reference)
	}
	if !strings.Contains(searchResult.Text, reference) || !strings.Contains(searchResult.Text, "media_type=music, and album_id=77") ||
		!strings.Contains(searchResult.Text, "(12 seeders / 1 leechers)") {
		t.Fatalf("search text = %q", searchResult.Text)
	}
	candidateJSON, err := json.Marshal(searchResult.ReleaseCandidates)
	if err != nil {
		t.Fatalf("encode safe candidates: %v", err)
	...
	grabInput, err := json.Marshal(map[string]any{
		"media_type": "music", "guid": reference, "indexer_id": 9, "album_id": 77,
	})
	if err != nil {
		t.Fatalf("encode grab input: %v", err)
	}
	...
	if got := searches.Load(); got != 2 {
		t.Fatalf("release searches = %d, want initial + fresh pre-dispatch search", got)
	}
}
