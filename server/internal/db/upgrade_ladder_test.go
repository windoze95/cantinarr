...
func TestOpenUpgradesOldestShippedSchema(t *testing.T) {
	...
	for _, table := range []string{"issues", "notification_prefs", "user_default_instances", "webauthn_credentials", "user_ai_settings", "arr_queue_witness", "content_alert_claims", "user_media_server_accounts", "user_content_policies", "apple_tv_devices", "apple_tv_grants"} {
		var count int
		if err := database.QueryRow(`SELECT COUNT(*) FROM ` + table).Scan(&count); err != nil {
