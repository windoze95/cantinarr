...
// fakeSignIn is an Emby that only knows how to sign a person in and out,
...
func TestAuthenticateSendsNoKeyThenLogsOut(t *testing.T) {
	...
	auth := f.authHeader.Get("X-Emby-Authorization")
	if !strings.HasPrefix(auth, "MediaBrowser ") || !strings.Contains(auth, `Client="Cantinarr"`) ||
		...
		t.Errorf("sign-in X-Emby-Authorization = %q", auth)
	}
	if strings.Contains(auth, "Token=") || strings.Contains(auth, testAPIKey) {
		t.Errorf("sign-in check carried a token: %q", auth)
	}
	for _, header := range []string{"X-Emby-Token", "Authorization"} {
		if got := f.authHeader.Get(header); got != "" {
			t.Errorf("sign-in check carried %s %q", header, got)
		}
	}
