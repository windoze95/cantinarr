	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assertExactMapKeys(t, payload,
				...
				"plex_access_requestable", "media_account_management", "admin_catalog_browsing", "hidden_discover_tabs", "apple_tv_remote", "tv_match_corrections", "tv_library_navigation", "request_quotas", "downloads_activity", "downloads_user_scope", "cover_4k_badges",
			)

			if string(payload["apple_tv_remote"]) != "false" {
				t.Fatal("unwired TV handler advertised support")
			}
			if string(payload["cover_4k_badges"]) != "false" {
				t.Fatal("4K badges on without an admin turning them on")
			}
