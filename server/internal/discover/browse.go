...
func discoverQuery(r *http.Request, spec discoverSpec, englishOnly bool) (url.Values, error) {
	params := url.Values{}
	...
	for _, k := range spec.dateKeys {
		if v := params.Get(k); v != "" {
			if _, err := time.Parse("2006-01-02", v); err != nil {
				return nil, fmt.Errorf("invalid %s: want YYYY-MM-DD", k)
			}
		}
	}
...
func (h *Handler) discover(w http.ResponseWriter, r *http.Request, spec discoverSpec) {
	...
	params, err := discoverQuery(r, spec, englishOnly)
	if err != nil {
		http.Error(w, fmt.Sprintf(`{"error":%q}`, err.Error()), http.StatusBadRequest)
		return
	}
	h.cachedTMDBFeed(w, spec.keyPrefix+params.Encode(), ttlTrending, spec.path, params)
}
