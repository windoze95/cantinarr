...
- **Push notifications** -- APNs (iOS) and FCM (Android) via a self-hosted push gateway with zero-config auto-enrollment: new-content alerts for movies, episodes, and books, approval/issue alerts for admins, server-wide controls and personal opt-outs under **Settings > Notifications > Push Notifications**, with separate optional auto-approved request alerts, access notifications for Plex/Jellyfin/Emby/Audiobookshelf, and deep links into the right screen.
