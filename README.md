...
- **TMDB + Trakt for movie and TV discovery** -- The best metadata, images, and trending data, proxied through the server so keys stay off devices -- and TMDB works out of the box on a built-in key, no signup needed. Sonarr's TVDB dependency is invisible. Every discovery row keeps loading and opens into a full grid, and a Browse page filters by genre, release year, and rating.
- **You choose what "popular" means** -- The headline row on the Movies and TV tabs reads TMDB weekly trending, or Trakt trending (ranked by who is actually watching), or TMDB's all-time popularity ranking. Connect Trakt and the rows switch to it automatically -- no second setting to find. An English-only switch keeps movie and TV discovery and recommendation rows to English-language originals; it ships on, and search always finds everything either way.
- **Automatic ID bridging** -- TMDB-to-TVDB translation with Trakt fallback. The #1 source of failed Sonarr adds, solved.
- **Browse before setup** -- Admins start with Movies, TV Shows, Releases, Books, and Music. Explore catalogs and open details before connecting a service. Each unconfigured catalog has a fixed **Set up Radarr/Sonarr/Chaptarr/Lidarr** and **Hide this tab** footer; setup returns to the same catalog or title. Hiding affects everyone on the server, and the tab returns automatically when its service is configured, even if that service is offline. Restore or change these choices under **Settings > Modules > Discover > Discover tabs**. Releases hides only when Movies, TV Shows, and Music are all hidden; an empty schedule does not hide it. Library rows, availability, and service-catalog search appear once the service is connected; requester and kids access still follows existing grants. Only visible book/music opening rows load in the background.
- **Books too** -- Search the selected Chaptarr library, then tap eBook, Audiobook, or **Request both**. The exact native book, library, and search term stay attached to the request. A tap saves and acknowledges immediately; delivery, approvals, and retries continue in the background. The Books tab opens with **Trending Books** -- what readers on [Hardcover](https://hardcover.app) are picking up right now, once an admin chooses **Connect Hardcover** in the Chaptarr instance settings and approves the one-time code (API tokens are also supported) -- then keeps **Recently Added**, **Authors**, and **Series**, all with live format availability. Books use Chaptarr directly; Open Library discovery and matching have retired. Requester access is granted per user. Book links can still open the provider pages recorded by Chaptarr.
- **Music too** -- One search for albums, EPs, singles, and artists, with live Lidarr status and saved requests attached by MusicBrainz ID. Albums appear as soon as they return; artists load independently below them. Search, discovery, artist discographies, and library navigation open the same album page and one request control. A tap saves immediately, while approvals and delivery continue in the background. Only the selected release is monitored. Popular Albums, New Releases, and genres stay focused on albums and EPs, followed by Recently Added and Artists. Music needs a per-user Lidarr grant for requesters; admins can browse before setup. See [music setup](docs/music-setup.md).
...
- **Requests survive outages** -- Movie, TV, book, and album requests save before delivery and retry temporary failures after a restart. Approval and delivery progress stay separate; each book format has its own progress. Retry a request that needs attention, or cancel it without discarding other people's shared requests. Unresolved requests from the retired book catalog retain their history and offer native search.
- **Request allowances** -- Optional rolling 1-, 7-, or 30-day allowances for movies, TV seasons, eBooks, audiobooks, and albums. Admins set User defaults and per-user exceptions in Request Defaults and user settings; kids inherit User defaults and admins are exempt. Allowance details live under Account settings; requests over the limit show a short toast naming the blocked category. All start unlimited.
- **Request approvals** -- Optional approval queue, globally or per user. Admins can search the server's request history from Approvals > History, including automatically approved requests. Admins also control per-user season choice, quality choice, and default quality profiles. Requesters can opt into push notifications for approvals and denials.
- **Discord notifications** -- Post request approvals, declines, availability, processing failures, and problem-report updates to a channel or thread. Users can opt in to personal mentions with multiple Discord IDs; admins choose events, role pings, and appearance. TV episodes are grouped to reduce noise, and delivery history makes failures visible. Open **Settings > Notifications > Discord Notifications**; native push is not required.
- **Works with apps built for Seerr** -- Cantinarr answers the Seerr API under `/api/v1`, so Maintainerr can see who requested a title before it cleans up (and clear the request when it does), Homepage can show request counts, and Dashbrr can list and approve requests, all without changes on their side. Issue the key under **Settings > Seerr-compatible API** and paste it where the app asks for Seerr, Overseerr, or Jellyseerr. Availability in those answers is read live from Radarr and Sonarr, the same as everywhere else in Cantinarr.
...
- **Push notifications** -- APNs (iOS) and FCM (Android) via a self-hosted push gateway with zero-config auto-enrollment: new-content alerts for movies, episodes, and books, approval/issue alerts for admins, server-wide controls and personal opt-outs under **Settings > Notifications > Push Notifications**, with separate optional auto-approved request alerts, access notifications for Plex/Jellyfin/Emby/Audiobookshelf, a simple "Ready to try again" when your own reported problem is resolved, and deep links into the right screen.
- **Plex, Jellyfin, and Emby access** -- connect your media server and choose which users get access. On Jellyfin or Emby each of them creates their own account from the app with a password only they know, or links one they already have by signing in with it once (administrator accounts included; Cantinarr never changes those); on Plex they sign in with their own Plex account, or share its email, and the invite goes out the moment you grant them (or on its own, with auto-approve). Already have users on the server? Import them from Users: each picked account becomes a Cantinarr user of the same name, granted and linked, with a connect link to hand out. New accounts and shares use the libraries you picked; existing accounts keep their libraries. Everyone sees the address to sign in at, an available title's page gets a **Watch on Plex** / **Watch on Jellyfin** / **Watch on Emby** button backed by a live lookup with their own access. By default, iOS and Android open the installed media app (the title where supported, otherwise the app home), with the exact web title as fallback when the app cannot open; web and desktop keep the web title link. On iPhone/iPad, admins can choose Infuse or Browser per video server, and users can override each service in **Settings > Account > Video apps**. **Open in Infuse** finds the movie or show across Infuse’s connected libraries without selecting a particular server/copy or starting playback automatically; failed launches quietly use the original browser URL. When Plex cannot verify the title, **Open Plex** opens the app home on mobile or the configured sign-in address as fallback, and the app shows the live state: invite pending, accepted, or switched off. Accounts and shares Cantinarr creates are managed automatically: removing their grant disables the account or removes the Plex share, and granting again restores access. Existing accounts linked or imported into Cantinarr are **Linked only** by default. Admins can explicitly manage their access, stop managing while retaining the link, or unlink without changing the remote account. Administrator and owner accounts are protected.
- **Open on Apple TV** -- pair TVs from Settings, choose which adults can use them, and open an available movie or series in Infuse from iOS, Android, or web. The server sends the title link without autoplay; a separate Confirm Open button handles the tvOS prompt when shown. [Apple TV setup](docs/apple-tv.md).
- **Audiobookshelf access and listening** -- share an Audiobookshelf server, let people create or link their own accounts, or import existing accounts from Users. Available audiobooks offer **Listen in …** after a live ISBN/ASIN match within the linked account's library and content access; separate copies stay separate for the listener to choose. **Open …** quietly opens the selected app or browser when an exact copy cannot be verified. Admins choose listening-app defaults per Audiobookshelf server; everyone can override them in **Settings > Account > Listening apps**, including ShelfPlayer on iPhone/iPad and TheShelf on Android. Failed app launches fall back to the browser. For accounts managed by Cantinarr, removing access disables the account without deleting listening history. Existing linked accounts stay unchanged unless an admin explicitly enables management. See [book setup](docs/books-setup.md#7-optional--listen-with-audiobookshelf).
...
- **Transcoding** -- a separate, admin-only, read-only module grouping Tdarr instances for active nodes and workers, current-step progress/FPS/ETA, and all-library or individual-library status counts. Supports multiple Tdarr servers; failed refreshes visibly mark the last successful snapshot as stale. No job controls or overall completion estimate.
...
│   └── internal/           # ai, api, arr, audiobookshelf, auth, cache, chaptarr, codexapp,
...
| Tdarr instance | Admin UI | Settings > Add Instance > Tdarr: name, server API URL (normally port 8266, not WebUI port 8265), and optional API key from Tools > API Keys when Tdarr authentication is enabled. The **Transcoding** menu groups these instances with admin-only Activity and Libraries tabs. Blank key edits preserve the saved key; **Remove saved API key** explicitly clears it |
| Plex, Jellyfin, Emby, or Audiobookshelf instance | Admin UI | Media server access: per-user grants, shared libraries, the address users open, and Audiobookshelf listening-app defaults; Plex links a plex.tv account with a PIN and picks the server to share |
...
| Discovery row source | Admin UI | Settings > Discovery: which feed backs the movie and TV headline rows (Trakt when configured, else TMDB trending), plus the English-only filter (on by default) |
...
3. Browse movies, TV shows, books, and music powered by TMDB, Trakt, ListenBrainz, MusicBrainz, Chaptarr, and Lidarr
...
8. Sharing a media server? Open **Watch on Plex** / **Watch on Jellyfin** / **Watch on Emby** in the menu: create your account or sign in with one you already have (Jellyfin, Emby), or sign in with Plex or share your Plex email and accept the invite (Plex), then sign in at the address shown; an available title's page in Cantinarr also has a **Watch on** button when the media server confirms the match, or **Open Plex** when an exact Plex link cannot be verified. Audiobook listeners use **Audiobookshelf access** (or **Media server access** when video servers are also shared) to create or link their account, then **Listen in …** on available books; choose a listening app in **Settings > Account > Listening apps** or follow the admin default

### For Admins
1. Deploy the container and complete the setup wizard
2. Add your shared API credentials and service instances from Settings; for included AI, either add an Anthropic/OpenAI/Gemini/xAI key or link a shared OpenAI (OAuth) or xAI Grok (OAuth) account; a Plex, Jellyfin, Emby, or Audiobookshelf instance also chooses which users get access there
...
| Discovery | TMDB API v3, Trakt API v2, ListenBrainz, MusicBrainz, Cover Art Archive (server-proxied) |
