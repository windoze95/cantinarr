...
- **Always in sync** -- availability is computed live from the arrs (never from a stale snapshot), and optional per-instance webhooks (edit an instance, copy one URL into Radarr/Sonarr > Connect) push manual imports, deletes, and adds into the app the moment they happen.
