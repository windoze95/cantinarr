...
- **Tdarr progress** -- a separate, admin-only, read-only module for active nodes and workers, current-step progress/FPS/ETA, and all-library or individual-library status counts. Supports multiple Tdarr servers; failed refreshes visibly mark the last successful snapshot as stale. No job controls or overall completion estimate.
...
| Tdarr instance | Admin UI | Settings > Add Instance > Tdarr: name, server API URL (normally port 8266, not WebUI port 8265), and optional API key from Tools > API Keys when Tdarr authentication is enabled. Activity and Libraries tabs are admin-only. Blank key edits preserve the saved key; **Remove saved API key** explicitly clears it |
