...
- **Books too** -- A Chaptarr (Readarr-API) module with per-format smarts: request the ebook, the audiobook, or both; likely matches are ranked automatically, with a compact match chooser only when the catalog contains a meaningful ambiguity. The chosen external author and publication identity travel with the request through approval and restart, and Cantinarr refuses to search if that exact choice can no longer be proved in Chaptarr. It verifies the selected local edition and a search acknowledgement before showing **Requested**. Owned-aware search and plain per-format controls stay pinned to the selected, authorized Chaptarr instance. Access is granted per user.
...
Download controls are enabled per instance. Instances that predate per-instance mappings retain the former identity behavior -- each configured global root is treated as both the arr path and Cantinarr path -- until explicit mappings are saved. New instances start with media downloads off. Cantinarr accepts only live file IDs from a user's effective Radarr/Sonarr instance or granted Chaptarr instance, refuses files outside that instance's mappings and the global roots, and gives the app a short-lived file-scoped link so large files stream through the browser or operating system without buffering in Flutter. The feature covers the primary files indexed by the arrs, not arbitrary files, subtitles, or extras found on disk.
