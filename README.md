...
- **Audiobookshelf access and listening** -- share an Audiobookshelf server, let people create or link their own accounts, or import existing accounts from Users. Available audiobooks offer **Listen in Audiobookshelf** after a live ISBN/ASIN match within the linked account's library and content access; separate copies stay separate for the listener to choose. **Open Audiobookshelf** is the general shortcut when an exact copy cannot be verified. For accounts managed by Cantinarr, removing access disables the account without deleting listening history. Existing linked accounts stay unchanged unless an admin explicitly enables management. See [book setup](docs/books-setup.md#7-optional--listen-with-audiobookshelf).
...
| Plex, Jellyfin, Emby, or Audiobookshelf instance | Admin UI | Media server access: per-user grants, shared libraries, and the sign-in address users see; Plex links a plex.tv account with a PIN and picks the server to share |
