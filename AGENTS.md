...
- Android release builds happen only in CI too (`playstore.yml`, builds a signed AAB on `main` when Android-relevant `app/**` paths change — web/ios/desktop subdirs, `app/test/**`, `app/tool/**`, markdown files, and store-listing metadata are excluded — and uploads it to the Play beta track when `PLAY_SERVICE_ACCOUNT_JSON` is set). PRs that touch `app/android/**`, `app/pubspec.yaml`, or the workflow get a build-only check (no upload). No local Android SDK is assumed; let CI prove the build.
