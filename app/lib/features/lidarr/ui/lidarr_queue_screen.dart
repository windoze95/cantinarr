...
class LidarrQueueScreen extends ConsumerStatefulWidget {
  const LidarrQueueScreen({super.key});

  @override
  ConsumerState<LidarrQueueScreen> createState() => _LidarrQueueScreenState();
}

class _LidarrQueueScreenState extends ConsumerState<LidarrQueueScreen> {
  List<LidarrQueueItem> _queue = [];
  ...
  Widget build(BuildContext context) {
    ...
    final wsInstanceId =
        ref.watch(instanceProvider.select((s) => s.activeLidarrInstance?.id));
    if (wsInstanceId != null) {
      ref.listen(
          arrQueueChangedProvider(
              (instanceId: wsInstanceId, serviceType: 'lidarr')), (_, next) {
