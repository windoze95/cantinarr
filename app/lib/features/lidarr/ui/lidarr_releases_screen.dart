...
/// Interactive release search for one album.
/// Queries all indexers live (slow), lists releases and lets the admin grab one
/// manually. Mirrors [SonarrReleasesScreen].
class LidarrReleasesScreen extends ConsumerStatefulWidget {
  final String instanceId;
  final int albumId;
  final String? albumTitle;

  const LidarrReleasesScreen({
    super.key,
  ...
  @override
  ConsumerState<LidarrReleasesScreen> createState() =>
      _LidarrReleasesScreenState();
}

class _LidarrReleasesScreenState
    extends ConsumerState<LidarrReleasesScreen> {
  ...
  Future<void> _search() async {
    ...
    try {
      final releases = await _service.getReleases(widget.albumId);
      if (!mounted) return;
      setState(() {
        _releases = releases;
        _isLoading = false;
  ...
  List<LidarrRelease> get _sorted {
  ...
  static int _smartCompare(LidarrRelease a, LidarrRelease b) {
  ...
  Future<void> _confirmGrab(LidarrRelease release) async {
      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            ...
            if (widget.albumTitle != null && widget.albumTitle!.isNotEmpty)
              Text(
                widget.albumTitle!,
