...
/// Opens the Import Doctor for a stuck Lidarr download. Returns `true` when a
/// fix was applied so the caller can refresh its queue.
Future<bool?> showLidarrImportDoctorSheet(
  BuildContext context, {
  required LidarrApiService service,
  required LidarrQueueItem item,
}) {
  return showAppSheet<bool>(
    context,
    builder: (_) => _LidarrImportDoctorSheet(service: service, item: item),
  );
}

/// The "Import Doctor": explains why a download is stuck (full transparency,
/// including the raw Lidarr messages) and offers one-click fixes. Destructive
/// actions (remove / blocklist / hand-off) ask for confirmation first.
class _LidarrImportDoctorSheet extends StatefulWidget {
  final LidarrApiService service;
  final LidarrQueueItem item;

  const _LidarrImportDoctorSheet({required this.service, required this.item});

  @override
  State<_LidarrImportDoctorSheet> createState() =>
      _LidarrImportDoctorSheetState();
}

class _LidarrImportDoctorSheetState
    extends State<_LidarrImportDoctorSheet> {
  LidarrApiService get _service => widget.service;
  bool _busy = false;
    ...
    switch (action) {
      ...
      case DoctorAction.rescan:
        if (item.artistId == null) {
          _toast('No artist to rescan.');
          return;
        }
        await _run(() => _service.rescanArtist(item.artistId!),
            'Rescanning the artist…');
      ...
      case DoctorAction.blocklistSearch:
        ...
        await _run(() async {
          ...
          if (item.albumId != null) {
            await _service.searchAlbums([item.albumId!]);
          }
        }, 'Blocklisted and searching for a replacement…');
      case DoctorAction.changeCategory:
        if (!await _confirm(
            'Hand off to download client',
            'This stops Lidarr managing the download and moves it to the post-import category (for tools like Unpackerr). It stays in your client.',
            'Hand off')) {
          return;
        }
        await _run(
  ...
  Future<void> _runManualImport({required bool force}) async {
    ...
    setState(() => _busy = true);
    List<LidarrManualImportCandidate> candidates;
    try {
    ...
    if (importable.isEmpty) {
      _toast(force
          ? "None of the files are matched to an album's tracks, so they cannot be imported."
  ...
  Widget build(BuildContext context) {
    final item = widget.item;
    final diagnosis = diagnoseLidarrQueueItem(item);
    ...
    return AppSheet(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(item.albumLabel ?? item.title,
              style: const TextStyle(
...
class _CandidatesDialog extends StatelessWidget {
  final List<LidarrManualImportCandidate> candidates;
                      ...
                      children: [
                        ...
                        Padding(
                          ...
                          child: Text(
                            [
                              c.sizeFormatted,
                              if (c.isMapped) 'matched' else 'no album match',
                            ].join(' • '),
                            style: const TextStyle(
