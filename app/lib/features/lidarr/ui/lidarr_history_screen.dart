...
/// Paginated Lidarr history: grabs, imports, failures, deletions, retags.
class LidarrHistoryScreen extends ConsumerStatefulWidget {
  const LidarrHistoryScreen({super.key});

  @override
  ConsumerState<LidarrHistoryScreen> createState() =>
      _LidarrHistoryScreenState();
}

class _LidarrHistoryScreenState extends ConsumerState<LidarrHistoryScreen> {
  static const _pageSize = 50;

  final _scrollController = ScrollController();
  List<LidarrHistoryRecord> _records = [];
  int _totalRecords = 0;
  int _page = 1;
...
/// Icon, colour and label for a Lidarr history event type.
({IconData icon, Color color, String label}) lidarrHistoryEventStyle(
    String eventType) {
  switch (eventType) {
    case 'grabbed':
      return (
...
class _HistoryTile extends StatelessWidget {
  final LidarrHistoryRecord record;

  const _HistoryTile({required this.record});

  @override
  Widget build(BuildContext context) {
    final style = lidarrHistoryEventStyle(record.eventType);
    final subtitleParts = [
      style.label,
