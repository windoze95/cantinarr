...
class LidarrModuleShell extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return ModuleScaffold(
      appBar: instanceState.lidarrInstances.length > 1
          ? AppBar(
              title: InstanceDropdown(
                instances: instanceState.lidarrInstances,
                activeInstanceId: instanceState.activeLidarrInstanceId,
                onChanged: (id) => ref
                    .read(instanceProvider.notifier)
                    .setActiveLidarrInstance(id),
              ),
              backgroundColor: AppTheme.background,
              elevation: 0,
            )
          : null,
      pages: modulePagesFor(ModuleType.lidarr),
