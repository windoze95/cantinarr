...
- **Season cards count the whole season** -- "11/13 Episodes Available • 2 unaired" rather than Sonarr's aired-and-monitored fraction, which calls a season with episodes still to come 100% complete. The remainder is split into buckets that add back up to the denominator -- **downloading** (or **waiting to import** once every one of them has finished transferring), **missing** for aired episodes nothing is working on, and **unaired**/**unmonitored** for the rest -- so an episode already in the queue is never reported as missing. Green is reserved for a season that is genuinely all on disk.
...
- **Discovery** (admin) -- picks which feed backs the headline discovery row, and offers an English-only switch that hides titles whose original language is not English from the discovery and recommendation rows. Search is never filtered, and a title the metadata source did not classify is kept rather than hidden. The Trakt source is shown but unselectable until a Trakt client ID is configured under Credentials.
