...
- **Books tab** -- appears only for users with a Chaptarr grant: owned-aware book search with per-format request buttons, confidence-ranked duplicate results, and tappable rows that open a richer book detail (see Books).
...
- **Book formats** -- request the eBook, the Audiobook, or both; each format continuously translates live Chaptarr truth into Available / Downloading / Requested / Pending approval / Request denied / Not requested. Requested means the exact format has a verified search (a leftover monitor flag is not enough), failures stay honest, and only the still-open format actions are shown. The request flow asks for a catalog choice only when distinct records or publications are genuinely ambiguous, labels the closest title/author match without claiming it is verified, shows publication evidence and every distinguishable alternative, carries the successful catalog query and exact external choice through approval/restart, and checks authoritative status after an interrupted response instead of inviting a duplicate tap.
- **Available-file downloads** -- when enabled for the selected media instance, an exact live movie, eBook, or audiobook file can be downloaded from its requester detail; TV seasons present their available episodes as individual choices so each tap starts only one download. A global deployment allowlist and the instance's own path mappings must both cover the file.
...
- **Owned-aware search** -- library titles are injected into lookup results with format-specific Available / Requested summaries; requestable results are confidence-ranked, while distinct records are never merged or silently discarded. If Chaptarr returns an empty list for a complete title, Cantinarr tries one narrower prefix lookup and keeps only rows that still strongly match the full query. When several records or publications could plausibly be the intended work, the request sheet shows the evidence and labels the closest match before submitting the selected foreignBookId, the query that found it, and stable external author/publication selectors. Cantinarr re-resolves those selectors against authoritative local editions and fails closed instead of silently substituting another version; Chaptarr-local numeric IDs never leave the server.
...
- **Instances** (admin) -- add/edit all eight service types; test connections; set the global default (single-default invariant with takeover confirmation) or per-user default pins; assign users to a Chaptarr instance (the Books access grant); assigning a user pinned to a sibling instance asks for confirmation before removing them from it. Radarr, Sonarr, and Chaptarr forms also accept repeatable media path mappings from the path that instance reports to a read-only, server-approved Cantinarr path; no mappings means downloads are off for a new instance. Use **Configure instant updates** to have the server rotate credentials and install the Radarr/Sonarr Connect webhook without exposing its secret to the device.
