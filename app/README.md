...
- **Owned-aware search** -- library titles are injected into lookup results with format-specific Available / Requested summaries; requestable results are confidence-ranked, while distinct records are never merged or silently discarded. When several records or publications could plausibly be the intended work, the request sheet shows the evidence and a recommendation before submitting the selected foreignBookId plus stable external author/publication selectors. Cantinarr re-resolves those selectors against authoritative local editions and fails closed instead of silently substituting another version; Chaptarr-local numeric IDs never leave the server.
