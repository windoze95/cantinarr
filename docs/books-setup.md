...
Publication details show edition publisher/format when available. Different editions may have different page counts. A background lookup with missing fields keeps the selected edition's existing details, and a missing date does not erase a year already supplied by the matching library record. Dates more than five years ahead appear as **Date unconfirmed (year)** and move to the undated end of an author's bibliography; Cantinarr keeps the source value rather than inventing a correction.
