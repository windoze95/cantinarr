...
4. Under **User Access**, grant the people who should listen. For each person, choose **Use server default**, **All libraries**, or **Choose libraries**. For example, Julian can receive `audiobooks` and Yana can receive `audiobooks-yana` on the same ABS server. An individual selection must contain at least one library. They also need their own Chaptarr access to see the books in Cantinarr. Neither service grants the other, and Audiobookshelf has no global default.
