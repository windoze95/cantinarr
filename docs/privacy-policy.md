...
- **ChatGPT or xAI in your browser, only when you choose to connect an OAuth provider.** **Settings > AI Access** can open OpenAI's or xAI's device-authorization page and show a one-time code for your personal provider. An admin can use the same explicit flows for an included server provider. The person linking the account signs in and approves it on the provider's page; the account password never passes through the Cantinarr app or server.
