...
- [ ] `PLEX-087` · P0 · UI/LIVE — Untick a granted user with an accepted share; verify the share disappears from Manage Library Access and the guide shows no account. Tick them again; verify a new invite goes out, must be accepted again, and the recipient's libraries are the instance's current selection.
