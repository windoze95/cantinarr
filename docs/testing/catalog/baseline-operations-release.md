...
- [ ] `OPS-015` · P2 · LIVE — Set an outbound proxy (a VPN-tunnelled Privoxy/3proxy) under Settings > Outbound Proxy; verify TMDB, Trakt, hosted AI, plex.tv, update-check, and push-relay traffic exits via the proxy (its access log) while arr, download-client, Jellyfin/Emby, and Local AI calls stay direct; verify Test reports a wrong port and a wrong password with the server's reason; verify `HTTP_PROXY` alone gives the same split with no `NO_PROXY`.
