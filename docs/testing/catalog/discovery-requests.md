...
- [ ] `DISC-010` · P0 · UI — Open movie detail; verify the hero shows the artwork/title without a generic eyebrow label, and rating, tagline, genres, overview, request/status actions, admin arr link, trusted trailer, recommendations, and similar titles map to that movie with safe missing-field fallbacks.
- [ ] `DISC-011` · P0 · UI — Open TV detail; verify the hero shows the artwork/title without a generic eyebrow label, and rating, tagline, genres, overview, real-season availability/request controls, admin arr link, trailer, recommendations/similar; requester season selection deliberately excludes Specials.
